...
# ==========================================
# پەڕەی مەسرووفات (دەستکاریکراو بەپێی خشتەکەت)
# ==========================================
WEB_MASRWF_TEMPLATE = """
<!DOCTYPE html>
<html lang="ckb" dir="rtl">
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Noto Kufi Arabic', sans-serif; }
        ...
        .masrwf-layout { display: grid; grid-template-columns: 1fr 390px; flex: 1; min-height: calc(100vh - 58px); }
        ...
        <aside class="input-sidebar">
            <form id="masrwfForm" method="POST" action="/admin/save_masrwf">
                ...
                <div class="field-group">
                    ...
                    <input type="text" id="txt_name" name="masrwf_name" class="c-input" placeholder="بۆ نموونە: کڕینی سەوزە، پسولەی کارەبا...">
                </div>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Noto Kufi Arabic', sans-serif; }
        ...
        .form-input { padding: 9px 12px; background: #03261d; border: 1.5px solid #0b5e4a; border-radius: 8px; color: #fff; font-size: 13px; outline: none; width: 100%; }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Noto Kufi Arabic', sans-serif; }
        ...
        .tables-flow-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 16px; margin-top: 20px; }
        .table-card { background-color: #10b981; border: 2px solid #047857; border-radius: 14px; height: 160px; display: flex; flex-direction: column; justify-content: space-between; cursor: pointer; text-align: center; color: #fff; }
        ...
        .modal-content { background: #ffffff; border-radius: 16px; width: 100%; max-width: 600px; padding: 24px; color: #0f172a; }
    </style>
</head>
<body>
    <header class="cashier-nav">
        <div style="font-size:20px; font-weight:800; color:#10b981;">✨ شاهور ڕێستۆرانت - کاشێر</div>
        <a href="/admin" class="btn-dash-back">⬅️ داشبۆرد</a>
    </header>
    <main class="cashier-container">
        <div class="tables-flow-grid">
            {% for t in active_tables %}
            <div class="table-card" onclick="openCheckout('{{ t.table_cabin }}')">
                <div style="font-size:30px; margin-top:10px;">🍽️</div>
                <div style="font-size:18px; font-weight:800;">مێزی {{ t.table_cabin }}</div>
                <div style="background:#047857; padding:8px; border-radius:0 0 12px 12px; font-weight:bold;">واصڵکردن ➔</div>
            </div>
            {% else %}
            <div style="grid-column: 1 / -1; text-align:center; padding:50px; color:#b45309; font-weight:bold; font-size:16px;">هیچ مێزێکی داواکراو نییە</div>
            {% endfor %}
        </div>
    </main>
    ...
    <div class="modal-overlay" id="checkoutModal">
        <div class="modal-content">
            <h3 id="checkoutTitle" style="margin-bottom:12px;">واصڵکردن</h3>
            <div id="checkoutItems" style="max-height:200px; overflow-y:auto; margin-bottom:12px;"></div>
            <div style="font-size:18px; font-weight:800; color:#059669; margin-bottom:12px;" id="lblTotalSum">کۆ: 0</div>
            <button onclick="submitPayment()" style="width:100%; padding:12px; background:#10b981; color:#fff; border:none; border-radius:8px; font-weight:bold; cursor:pointer;">واصڵکردن و بەتاڵکردنی مێز</button>
            <button onclick="document.getElementById('checkoutModal').style.display='none'" style="width:100%; padding:8px; background:none; border:none; color:#64748b; margin-top:6px; cursor:pointer;">داخستن</button>
        </div>
    </div>
    ...
    <script>
        let curTable = '';
        let totVal = 0;
        function openCheckout(tbl) {
            curTable = tbl;
            document.getElementById('checkoutTitle').innerText = 'واصڵکردنی: ' + tbl;
            fetch('/get_table_orders/' + encodeURIComponent(tbl)).then(r => r.json()).then(items => {
                let html = ''; totVal = 0;
                items.forEach(i => {
                    let st = i.price * i.quantity;
                    totVal += st;
                    html += `<div style="display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px solid #e2e8f0;"><span>${i.food_name} (x${i.quantity})</span><span>${st.toLocaleString()} د.ع</span></div>`;
                });
                document.getElementById('checkoutItems').innerHTML = html;
                document.getElementById('lblTotalSum').innerText = 'کۆی گشتی: ' + totVal.toLocaleString() + ' دینار';
        ...
        function submitPayment() {
            fetch('/admin/complete_payment', {
                ...
                body: JSON.stringify({ table_number: curTable, amount_paid: totVal, total_amount: totVal })
...
# ==========================================
# پەڕەی بەڕێوەبردنی مێنۆ (Menu Manager)
# ==========================================
...
<body>
    ...
    <table>
        <thead><tr><th>#</th><th>کاتی وەسڵ</th><th>شوێن / مێز</th><th>بڕی پارە</th></tr></thead>
        <tbody>
            {% for r in qasa_rows %}
            <tr><td>{{ loop.index }}</td><td>{{ r.transaction_time }}</td><td>{{ r.place_id }}</td><td style="color:#10b981; font-weight:bold;">{{ "{:,.0f}".format(r.amount) }} د.ع</td></tr>
            {% else %}<tr><td colspan="4">هیچ تۆمارێک نییە</td></tr>{% endfor %}
        </tbody>
    </table>
</body>
</html>
...
# ==========================================
# ئامار و قازانج (بەستراو بە تەواوی داتابەیسەکە)
# ==========================================
...
def admin_amar():
    ...
    try:
        conn = get_db()
        with conn.cursor() as cur:
            # 1. هێنانی فرۆشی خواردنەکان لە خشتەی froshtn
            query_sales = """
            ...
            # 2. هێنانی مەسرووفات لە خشتەی masrwf بەپێی DATE(masrwf_date)
            try:
            ...
            # 3. هێنانی حیساباتی شاگردەکان لە worker_attendance
            try:
                query_workers = """
...
# ==========================================
# مەسرووفات (پشکنین و هەڵگرتنی تەواوی ستوونەکان)
# ==========================================
...
# ==========================================
# ئایپاد، مۆبایل و بینینی موشتەری
# ==========================================
